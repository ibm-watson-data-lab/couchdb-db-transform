...
Replicator.prototype.init = function(initCallback) {
	...
	if((! this.sourceCredentials) || (! this.sourceCredentials.url) || (! this.sourceCredentials.dbname)) {
		return initCallback('Error. Source database information is missing or incomplete. Expected input format https://$USERNAME:$PASSWORD@$REMOTE_USERNAME.cloudant.com/$SOURCE_DATABASE_NAME');
	}

	if((! this.targetCredentials) || (! this.targetCredentials.url) || (! this.targetCredentials.dbname)) {
		return initCallback('Error. Target database information is missing or incomplete. Expected input format https://$USERNAME:$PASSWORD@$REMOTE_USERNAME.cloudant.com/$TARGET_DATABASE_NAME');
	}

	// connect to source system	
	const sourceCloudant = require('cloudant')({url:this.sourceCredentials.url});

	// verify that the source database exists
	sourceCloudant.db.get(this.sourceCredentials.dbname, function(err, body) {
		...
		const sourceDb = sourceCloudant.db.use(this.sourceCredentials.dbname);
	...
	}.bind(this)); // sourceCloudant.db.get(this.sourceCredentials.dbname,...
